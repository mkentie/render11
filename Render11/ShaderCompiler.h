...
protected:
    ...
    template<class T>
    using ShaderCreationFunc = HRESULT(__stdcall ID3D11Device::*)(const void*, SIZE_T, ID3D11ClassLinkage*, T**);

    template<class T, ShaderCreationFunc Func>
    ComPtr<T> CompileXShader(const char* const pszEntryPoint, const char* const pszTarget);
